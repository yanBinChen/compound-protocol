    ...
    ExponentialNoError
{
    ...
    function borrowAllowed(
        ...
        if (!markets[cToken].accountMembership[borrower]) {
            // 预期你借款之前，需要先进入这个市场，第一次贷款需要有个approve步骤，允许进入这个市场
            // 并且允许合约操作指定数量的资产，用于清算流程
        ...
        // Keep the flywheel moving
        // 在 Solidity 中，public 状态变量会自动生成一个同名的 getter 函数。
        // 例如，borrowIndex 状态变量会生成一个隐式的函数：
     ...
     */
    //  转账和redeem类似，需要保证转出去之后，账户的保证金/抵押品的价值足够
    ...
    function getHypotheticalAccountLiquidityInternal(
        ...
        for (uint i = 0; i < assets.length; i++) {
            ...
            if (asset == cTokenModify) {
                // 当前遍历的资产如果是这个交易本身想要redeem或者borrow的资产
                // redeem effect
